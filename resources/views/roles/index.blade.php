                                     ...
                                     <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        <tr>
                                           ...
                                           <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                              <a href="{{ route('roles.show', $role->id) }}" class="inline-block rounded-full
                                                text-xs font-bold mr-4 opacity-90 hover:opacity-100"><x-heroicon-s-eye class="h-5 w-5 text-green-700" /></a>
                                                 @can('role_edit')
                                              <a href="{{ route('roles.edit', $role->id) }}" class="inline-block rounded-full mr-4"><x-heroicon-s-pencil-alt class="h-5 w-5 text-purple-700" /></a>
                                                 @endcan
                                                 @can('role_delete')
                                                 <form class="inline-block" action="{{ route('roles.destroy', $role->id) }}" method="POST">
                                                 <input type="hidden" name="_method" value="DELETE">
                                                 <input type="hidden" name="_token" value="{{ csrf_token() }}">
                                                 <button type="submit" class="inline-block rounded-full text-xs font-bold mr-1 opacity-90 hover:opacity-100 delete-confirm"><x-heroicon-s-trash class="h-5 w-5 text-red-700" /></button></a>
                                              </form>
                                              @endcan
                                           </td>
                                        </tr>
