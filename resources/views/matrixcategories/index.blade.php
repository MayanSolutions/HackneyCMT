                            ...
                            <div class="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
                               <div class="shadow overflow-hidden border-b border-gray-200 ">
                                <h2 class="mb-2 mt-4 ml-6 font-extrabold float-left tracking-tight text-gray-700">
                                    Housing Services and Functions
                                  </h2>
                                     ...
                                     <tbody class="bg-white divide-y divide-gray-200">
                                        ...
                                        <tr>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-500">{{ $category->category }}</div>
                                             </td>
                                           <td class="px-6 py-4 flex flex-wrap max-w-sm">
                                              <div class="text-sm text-gray-500">{{ $category->description }}</div>
                                           </td>

                                           <td class="px-6 py-4 whitespace-nowrap">
                                            @if ($category->MatrixFunction->count() > 0)
                                            <div class="text-sm text-green-700">{{ $category->MatrixFunction->count() }} Registered</div>
                                            @else
                                            <div class="text-sm semi-bold text-red-700">No registered functions</div>
                                            @endif
                                            </td>

                                           <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                              <a href="{{ route('matrixcategories.show', $category->id) }}" class="inline-flex bg-white hover:bg-purple-600 text-gray-800 hover:text-white text-xs font-semibold py-1 px-2 border border-gray-400 rounded shadow">Show</a>
                                                 @can('can_edit_matrix_cat')
                                              <a href="{{ route('matrixcategories.edit', $category->id) }}" class="inline-flex bg-white hover:bg-orange-500 text-gray-800 hover:text-white text-xs font-semibold py-1 px-2 border border-gray-400 rounded shadow">Modify</a>
                                                @endcan
